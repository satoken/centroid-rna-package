...
struct CountBP
{
  ...
  const L& bp_;
  CentroidFoldBase::BPTable table;
  uint p;
};
...
// Standard folding routines
// 1. pf_fold: McCaskill model in Vienna RNA pacakge
// 2. pfl_fold: A banded version of McCaskill model in Vienna RNA package
// 3. alipf_fold: A variant of McCaskill model for multiple alignments in Vienna RNA Package
// 4. contra_fold: CONTRAfold model
...
// Stochastic folding routines
// 1. pf_fold_st: A stochastic version of McCaskill model in Vienna RNA package
// 2. pf_fold_st: A stochastic version of McCaskill model in Vienna RNA package with index conversion
// 3. pf_fold_st: A stochastic version of McCaskill model in Vienna RNA package with raw outputs
// 4. pf_fold_st: A stochastic version of McCaskill model in Vienna RNA package with raw outputs with index conversion
// 5. alipf_fold_st: A variant of stochastic version of McCaskill model for multple alignments in Vienna Package
// 6. alipf_fold_st: A variant of stochastic version of McCaskill model for multple alignments in Vienna Package for raw outpus
// 7. contra_fold_st: A stochastic version of CONTRAfold model
// 8. contra_fold_st: A stochastic version of CONTRAfold model for multiple alignmens
// 9. contra_fold_st: A stochastic version of CONTRAfold model with index conversion
// 10. contra_fold_st: A stochastic version of CONTRAfold model with raw outpus
// 11. contra_fold_st: A stochastic version of CONTRAfold model for multiple alignments with raw outpus 
// 12. contra_fold_st: A stochastic version of CONTRAfold model for multiple alignments with raw outpus with index conversion
...
// calculate expected accuracy using stochastic sampling
// 1. pf_fold_ea: McCaskill model in Vienna RNA package
// 2. pf_fold_ea: McCakkill model for multiple alignments in Vienna RNA package
// 3. alipf_fold_ea: McCakkill model for multiple alignments in Vienna RNA package
// 4. contra_fold_ea: CONTRAfold model
// 5. contra_fold_ea: CONTRAfold model for multiple alignments
// 6. contra_fold_ea: CONTRAfold model for multiple alignments (with averaged tracing back)

// Added by M. Hamada (2009/09/18)
static
void
compute_expected_accuracy (const std::string& paren,
                           const CentroidFoldBase::BPTable& bp, 
...
// predict a secondary structure whose MCC is best using stochastic sampling
// 1. pf_fold_max_mcc: McCaskill model in Vienna RNA package
// 2. pf_fold_max_mcc: McCakkill model for multiple alignments in Vienna RNA package
// 3. alipf_fold_max_mcc: McCakkill model for multiple alignments in Vienna RNA package
// 4. contra_fold_max_mcc: CONTRAfold model
// 5. contra_fold_max_mcc: CONTRAfold model for multiple alignments
// 6. contra_fold_max_mcc: CONTRAfold model for multiple alignments (with averaged tracing back)
		...
		std::ostream& out)
{
  ...
  for (uint n=0; n!=num_samples; ++n) {
    ...
    double sen, ppv, mcc;
    compute_expected_accuracy (paren, bp, sen, ppv, mcc);
    if (MCC<mcc) {
      PAREN = paren;
...
pf_fold_max_mcc(uint num_samples, const std::string& name, const std::string& seq,
                ...
                const CentroidFoldBase::BPTable& bp, std::ostream& out)
{
  int bk_st_back=Vienna::st_back;
  Vienna::st_back=1;
                    ...
                    const CentroidFoldBase::BPTable& bp, std::ostream& out)
{
  ...
  for (uint n=0; n!=num_samples; ++n) {
    ...
    double sen, ppv, mcc;
    compute_expected_accuracy (p, bp, sen, ppv, mcc);
    if (MCC<mcc) {
      PAREN = p;
      MCC = mcc; SEN = sen; PPV = ppv; 
    }
...
void
CentroidFold::
compute_expected_accuracy_sampling(const std::string& paren,  
