...
cbpd(int argc, char* argv[])
{
  ...
  desc.add_options()
    ...
    ("pairwise", "calculate the pairwise variant")
#ifdef HAVE_LIBRNA
      ...
      (vm.count("aux") && model.empty()))
  {
    ...
    std::cout << "CBPD v" << VERSION << std::endl
  ...
  while ((bytes=aln.load(fi))>0)
  {
    ...
    {
      std::vector<float> r = cf->cbpd_pairwise(aln, gamma_s);
      std::copy(r.begin(), r.end(), std::ostream_iterator<float>(std::cout, ","));
      std::cout << std::endl;
    }
    else
    {
      std::vector<float> r = cf->cbpd_consensus(aln, gamma_a, gamma_s);
      std::copy(r.begin(), r.end(), std::ostream_iterator<float>(std::cout, ","));
      std::cout << std::endl;
    }
  }
